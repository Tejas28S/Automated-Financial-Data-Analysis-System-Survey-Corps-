...
def _generated_txn_id(source_path: Path, source_bucket: str, row_number: int) -> str:
    identity = f"{source_path.resolve()}::{source_bucket}::{row_number}"
    digest = hashlib.sha256(identity.encode("utf-8")).hexdigest()[:16]
    return f"generated::{source_path.stem}::{row_number}::{digest}"
...
def _normalize_frame(
    ...
    for row_number, (_, row) in enumerate(frame.iterrows(), start=1):
        ...
        txn_id = _first_value(row, lookup, ("txn_id",)) or _generated_txn_id(
            source_path, row_bucket, row_number
        )
...
def load_inputs(input_path: str | Path) -> NormalizedInput:
    ...
    return NormalizedInput(transactions=transactions, input_manifest=manifest, metadata=metadata)
