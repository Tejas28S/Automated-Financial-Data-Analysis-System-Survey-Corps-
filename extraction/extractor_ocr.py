...
def run_groq_vision_on_image(image_path: str) -> str:
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        try:
            ...
            # Groq Vision accepts images as "data:{mime_type};base64,{b64_string}".
            # WHAT IS SENT: The image bytes (base64-encoded) + OCR prompt.
    ...
    try:
        ...
        for page_number in range(pages_to_process):
            try:
                ...
                # Create a transformation matrix that renders the page at 300 DPI.
                # The default screen resolution is 72 DPI. 300/72 ≈ 4.17 is the scale factor.
                # 300 DPI is the standard recommended resolution for OCR.
                zoom_factor = 300 / 72
                ...
                # We use a temporary file so we do not clutter the disk with
                ...
                # Delete the temporary image file now that we are done with it.
