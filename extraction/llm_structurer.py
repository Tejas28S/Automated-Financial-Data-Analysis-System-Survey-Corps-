...
# How many text lines to send per transaction-reading chunk, and a safety cap on
# the number of chunks so a huge document can never run unbounded (overheating).
CHUNK_LINES = 60
MAX_CHUNKS = 40
...
_TXN_PROMPT = (
    ...
    "date from a time. Keep the reference/cheque number OUT of the description. Use\n"
    "\"\" for anything not present. A row that wraps across lines is ONE transaction.\n"
)
...
def _call_json(system_prompt: str, user_text: str, max_tokens: int = 8000) -> Dict[str, Any]:
...
def structure_statement(raw_text: str, file_path: str = "") -> Dict[str, Any]:
    ...
    chunks = [lines[i:i + CHUNK_LINES] for i in range(0, len(lines), CHUNK_LINES)][:MAX_CHUNKS]
